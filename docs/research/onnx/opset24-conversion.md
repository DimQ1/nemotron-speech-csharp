...
> (`docs/research/onnx/opset24-analysis.md`): реальную выгоду даёт обновление рантайма ORT,
...
- `tools/converters/NemotronAsr/src/nemotron_model_load.py` — `CHUNK_SIZE` стал ленивым
  (`_chunk_size()` читает `NEMOTRON_CHUNK_SIZE` при обращении, не при импорте).
- `tools/converters/NemotronAsr/src/optimize.py`:
...
- `tools/converters/NemotronAsr/src/patch_olive_torch_export.py` — runtime-патч: torch 2.12+
  убрал kwarg `fallback` из `torch.onnx.export`, а Olive 0.8 его передаёт.
- `tools/converters/NemotronAsr/src/flatten_output.py` — раскладывает вложенный вывод Olive
  (`<comp>.onnx/output_model/model/model.onnx`) в плоскую структуру приложения.
- `tools/converters/NemotronAsr/src/quantize_int4.py` — прямая INT4-квантизация FP32 encoder через
